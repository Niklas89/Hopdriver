...
if(!empty($_SESSION['id']))
{
  ...
  include '../config.php';
   ...
   $req = $bdd->prepare('SELECT id,email FROM clients WHERE id=:id_users AND email = :email');
  $req->execute(array(
    'id_users'=>$id_users,
    'email'=>$email
  ));
...
if(isset($_POST['register']))  {
  if(!empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['email']) && !empty($_POST['pass'])
      ...
      if (preg_match("#^\S*(?=\S{6,30})(?=\S*[a-z])\S*$#", $_POST['pass']))
    {
            ...
            /*
            The requirements:
        if (preg_match("#^\S*(?=\S{6,30})(?=\S*[a-z])(?=\S*[\d]{2,30})\S*$#", $_POST['pass']))
      ...
      {
        $valid = false;
        $erreuridd = _('This member already exists.');
      }
    ...
    date_default_timezone_set('Europe/Madrid');
      ...
      $reqq = $bdd->prepare('INSERT INTO clients (pass,email,date_inscription,first_name,last_name) VALUES (:pass,:email,:date_inscription,:first_name,:last_name)');
      $reqq->execute(array(
        'pass'=>$pass,
        'email'=>$email,

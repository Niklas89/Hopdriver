                ...
                <div class="col-md-9">
                    ...
                    <ul class="list list-inline user-profile-statictics mb30">
                        <li><a href="transfers.php" title="<?php echo _('New Transfers'); ?>"><i class="fa fa-cab user-profile-statictics-icon"></i>
                            <?php 
                                 $sth = $bdd->query("SELECT COUNT(id) AS numid FROM transfers_booking WHERE clientid = ".$id_users);
                                $result = $sth->fetch(PDO::FETCH_OBJ);
                       ...
                       <li><a href="disposals.php" title="<?php echo _('New Disposals'); ?>"><i class="fa fa-car user-profile-statictics-icon"></i>
                            <?php 
                                 $sth = $bdd->query("SELECT COUNT(id) AS numid FROM disposal_booking WHERE clientid = ".$id_users);
                                $result = $sth->fetch(PDO::FETCH_OBJ);
                                $numtransfers = $result->numid;
                                $sth->closeCursor(); ?>
                            <h5>
                    ...
                    <table class="table table-bordered table-striped table-booking-history">
                        ...
                        <tbody>
                             <?php 
                              ...
                              $resultats=$bdd->query('SELECT * FROM transfers_booking WHERE  clientid = '.$id_users.' ORDER BY coldate DESC LIMIT 3');
                     ...
                     <div class="checkbox">
                        <label>
                            3 <?php echo _('latest disposals'); ?>:</label>
                    </div>
                    <table class="table table-bordered table-striped table-booking-history">
                        <thead>
                            <tr>
                        ...
                        <tbody> 
                             <?php 
                              ...
                              $resultats=$bdd->query('SELECT * FROM disposal_booking WHERE clientid = '.$id_users.' ORDER BY coldate DESC LIMIT 3');
                              $resultats->setFetchMode(PDO::FETCH_OBJ);
                              while( $resultat = $resultats->fetch() )
                              { 
                                ...
                                ?>
                                         ...
                                         <tr>
                                            ...
                                            <td><a class="btn btn-info btn-sm" href="disposaldetails.php?id=<?php echo $resultat->id; ?>" title="<?php echo _('More details'); ?>"><?php echo _('Infos'); ?></a></a></td>
                                            <?php if ($resultat->driverid == $id_users ) { ?> 
                                            <td class="text-center"><i class="fa fa-check"></i>
                                            </td>
                                            <?php } else { ?>

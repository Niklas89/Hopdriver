...
$retour_total=$bdd->query("SELECT driver_vehicles." . $services . " AS vehiclesservices,
                        ...
                        WHERE driver_prices." . $services."<>0 ORDER BY driver_prices." . $services);
...
<body>
    ...
    <div class="global-wrap">
        ...
        <div class="container">
            <ul class="breadcrumb">
                <li><a action="../index.php" title="Previous"><?php echo _("Home"); ?></a>
                </li> <!--
                <li><a href="#">France</a>
                </li>
                <li><a href="#">Paris</a>
            ...
            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" value="<?php if (!empty($_POST["pickupdate"])) { echo $pickupdate; } else { echo ""; } ?>" name="pickupdate" type="text" />
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" value="<?php if (!empty($_POST["drop_off_date"])) { echo $drop_off_date; } else { echo ""; } ?>" name="drop_off_date" type="text" />
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label><?php echo _("Drop-Off Time"); ?></label>
                                        <input class="time-pick form-control" value="<?php if (!empty($_POST["drop_off_time"])) { echo $drop_off_time; } else { echo $current_time; } ?>" name="drop_off_time" type="text" />
                                    </div>
                                </div>
                            </div>

                                 <?php if (isset($_POST["disposal-cars"]) || isset($_POST["services"])) {
                                ...
                                $drop_off_date = date("d M Y", strtotime($_POST["drop_off_date"]));
                                ...
                                foreach($data->rows[0]->elements as $road) {
                                 ...
                                 $sth = $bdd->prepare("SELECT MIN(disposal_ecocar) AS disposalecocar,
                                         MIN(disposal_ecovan) AS disposalecovan,
                                          MIN(disposal_buscar) AS disposalbuscar,
                                           MIN(disposal_busvan) AS disposalbusvan,
                                            MIN(disposal_luxcar) AS disposalluxcar,
                                             MIN(disposal_moto) AS disposalmoto,
                                              MIN(disposal_coach) AS disposalcoach
                                      FROM driver_prices");
                                        $sth->execute();
                                          ...
                                          MIN(disposal_buscar) AS disposalbuscar,
                                           MIN(disposal_busvan) AS disposalbusvan,
                                            MIN(disposal_luxcar) AS disposalluxcar,
                                             MIN(disposal_moto) AS disposalmoto,
                                              MIN(disposal_coach) AS disposalcoach
                                      FROM driver_prices WHERE disposal_ecocar <> 0 AND disposal_ecovan <> 0 AND disposal_buscar <> 0 AND disposal_busvan <> 0 AND disposal_luxcar <> 0 AND disposal_moto <> 0 AND disposal_coach <> 0");
                                        $sth->execute();
                                        $result = $sth->fetch(PDO::FETCH_OBJ);
                                        $sth->closeCursor();
                                        $economycar = $result->disposalecocar;
                ...
                <div class="col-md-9">
                   <!-- <div id="selectbrand">
                    <form  action="disposal-services-class.php" method="post" class="main-header-search">
                    ...
                    <ul class="booking-list">
                        ...
                        INNER JOIN driver_payment ON driver_payment.driverid = driver_prices.driverid
                        INNER JOIN driver_luggage ON driver_payment.driverid = driver_luggage.driverid
                        WHERE driver_prices." . $services."<>0 ORDER BY pricesdisposalservices ASC");
                      $resultats->setFetchMode(PDO::FETCH_OBJ);
                      while( $resultat = $resultats->fetch() ) { ?>

                        <li>
                            <form id="form-<?php echo $resultat->pricesdriverid; ?>" action="disposal-confirmation.php" method="post">
                                <input type="hidden" name="pick_up_loc" value="<?php echo $pick_up_loc; ?>" />
                                <input type="hidden" name="drop_off_loc" value="<?php echo $drop_off_loc; ?>" />
                                <input type="hidden" name="pickupdate" value="<?php echo $pickupdate; ?>" />
                                <input type="hidden" name="pickuptime" value="<?php echo $pickuptime; ?>" />
                                ...
                                <a class="booking-item" href="#" onclick="document.getElementById('form-<?php echo $resultat->pricesdriverid; ?>').submit();" title="<?php _('Select Disposal'); ?>">
                              ...
                              elseif($req->rowCount()==0) // check if row where there is this email doesn't exist
                                {
                                 echo $servicesimg;
                                    ...
                                    <div class="col-md-3"><span class="booking-item-price"><?php echo $resultat->pricesdisposalservices; ?>€</span><span>/<?php echo _('hour'); ?></span>
                                        <br /><span><?php echo nl2br($resultat->vehiclesservices); ?></span><br />
                                        <br /><span class="btn btn-primary"><?php echo _('Select'); ?></span>
                                    </div>
                                    <input type="hidden" name="price" value="<?php echo $resultat->pricesdisposalservices; ?>" />
                                    <input type="hidden" name="hoursday" value="<?php echo $hoursday; ?>" />
                                    <?php $totalprice = $totalhours*$resultat->pricesdisposalservices; ?>
    ...
    <script src="//maps.googleapis.com/maps/api/js?key=AIHVg&libraries=places&language=en"></script>

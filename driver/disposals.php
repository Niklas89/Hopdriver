...
$sth = $bdd->prepare("SELECT first_name, last_name, email, country FROM chauffeurs WHERE id = :id_users");
  ...
  if (isset($_GET['valid'])) {
    ...
    $req = $bdd->prepare('UPDATE disposal_booking SET driverid = :id_users, accepted = :accepted WHERE id=:id');
    $req->execute(array(
      'id_users' => $id_users,
      'accepted' => $accepted,
      'id' => $id
    ...
    $req->closeCursor();
        ...
        $stt = $bdd->prepare("SELECT * FROM disposal_booking WHERE driverid = :id_users");
        ...
        $pick_up_loc = $resultresa->pick_up_loc;
        $drop_off_loc = $resultresa->drop_off_loc;
        $drop_off_date = $resultresa->drop_off_date;
        $drop_off_time = $resultresa->drop_off_time;
        $phone = $resultresa->phone;
        $service = $resultresa->service;
        $hoursday = $resultresa->hoursday;
        $comments = $resultresa->comments;
        $price = $resultresa->price;
        $clientid = $resultresa->clientid;
        $vehicle = $resultresa->vehicle;

        if (empty($vehicle)) { $vehicle = _("Not Specified"); }
     ...
     $sth = $bdd->prepare("SELECT first_name, last_name, email, phone, phonetwo, languages FROM chauffeurs WHERE id = :id_users");
      ...
      /* NOT FUNCTIONABLE ANYMORE ON IMAGIDEV.COM
                  ...
                  $email_subject = _('Accepted Disposal on').' '.$pickupdate.' '. _('at').' '.$pickuptime.' '.$price.'€';
            ...
            try {
              $mailmessage = file_get_contents('acceptdisposal_'.$lang.'.html');
              ...
              $mail->Password   = "0a10a426affe8dc1";        // SMTP account password
            ...
            try {
              ...
              $mail->Host       = "ssl://in.mailjet.com"; // sets the SMTP server
              $mail->Port       = 443;                    // set the SMTP port for the GMAIL server
              $mail->Username   = "780189720e93c038a"; // SMTP account username
              $mail->Password   = "0a10a8affe8dc1";        // SMTP account password
              $mail->AddAddress($clientemail);
              $mail->SetFrom('contact@imagidev.com', 'HopDriver Bookings');
              $mail->AddReplyTo('contact@imagidev.com', 'HopDriver Bookings');
  ...
  if (isset($_GET['cancel'])) {
    ...
    $id_cancel = 0;
     ...
     $sth = $bdd->prepare("SELECT first_name, last_name, email FROM chauffeurs WHERE id = :id_users");
      ...
      /* NOT FUNCTIONABLE ANYMORE ON IMAGIDEV.COM
                  ...
                  $email_subject = _('Canceled Disposal on').' '.$pickupdate.' '. _('at').' '.$pickuptime;
            ...
            try {
              $mailmessage = file_get_contents('canceldisposal_'.$lang.'.html');
              $mailmessage = str_replace('%first_name%', $first_name, $mailmessage);  // http://www.xeweb.net/2009/12/31/sending-emails-the-right-way-using-phpmailer-and-email-templates/
              $mailmessage = str_replace('%last_name%', $last_name, $mailmessage);
              $mailmessage = str_replace('%driverfirst_name%', $driverfirst_name, $mailmessage);
              $mailmessage = str_replace('%pickupdate%', $pickupdate, $mailmessage);
              ...
              $mail->Password   = "0a10a8b426affe8dc1";        // SMTP account password
              $mail->AddAddress($driveremail);
              $mail->SetFrom('drivers@imagidev.com', 'HopDriver Drivers');
              $mail->AddReplyTo('drivers@imagidev.com', 'HopDriver Drivers');
              $mail->AddBCC('drivers@imagidev.com');
    ...
    $req = $bdd->prepare('UPDATE disposal_booking SET driverid = :id_cancel, accepted = :id_cancel WHERE id=:id');
    $req->execute(array(
      'id_cancel' => $id_cancel,
      'id' => $id
    ));
...
$retour_total=$bdd->query('SELECT COUNT(*) AS total FROM disposal_booking WHERE accepted=0 OR (accepted=1 AND driverid='.$id_users.') AND pick_up_loc LIKE "%'.$drivercountry.'%"');
...
<head>
    <title>HopDriver - <?php echo _('New Disposals'); ?></title>
...
<body>
    ...
    <div class="global-wrap">
        ...
        <div class="container">
            <h1 class="page-title"><?php echo _('New Disposals'); ?></h1>
        </div>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-9">
                    <div class="checkbox">
                        <label>
                            <!--<input class="i-check" type="checkbox" />--><?php echo _('Accept disposals in the list below'); ?>:</label>
                    </div>
                  ...
                  $premiereEntree=($pageActuelle-1)*$messagesParPage; // On calcul la première entrée à lire
                              ...
                              $retour_messages=$bdd->query('SELECT * FROM disposal_booking WHERE accepted=0 OR (accepted=1 AND driverid='.$id_users.') AND pick_up_loc LIKE "%'.$drivercountry.'%" ORDER BY id DESC LIMIT '.$premiereEntree.', '.$messagesParPage.'');
                              ...
                              while( $resultat = $retour_messages->fetch() )
                              {
                                ...
                                ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><a class="btn btn-info btn-sm" href="disposaldetails.php?id=<?php echo $resultat->id; ?>" title="More details"><?php echo _('Infos'); ?></a></td>

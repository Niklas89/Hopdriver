     ...
     $sth = $bdd->prepare("SELECT first_name, last_name, email, phone, phonetwo, languages FROM chauffeurs WHERE id = :id_users");
      ...
      $ok = "";
            ...
            try {
              ...
              $mail->Password   = "0a1e8dc1";        // SMTP account password
              $mail->AddAddress($driveremail);
              $mail->SetFrom('drivers@hopdriver.com', 'HopDriver Drivers');
              $mail->AddReplyTo('drivers@hopdriver.com', 'HopDriver Drivers');
              $mail->AddBCC('drivers@hopdriver.com');
            ...
            try {
              ...
              $mail->Host       = "ssl://in.mailjet.com"; // sets the SMTP server
              $mail->Port       = 443;                    // set the SMTP port for the GMAIL server
              $mail->Username   = "780172038a"; // SMTP account username
              $mail->Password   = "0a16affe8dc1";        // SMTP account password
              $mail->AddAddress($clientemail);
              $mail->SetFrom('bookings@hopdriver.com', 'HopDriver Bookings');
              $mail->AddReplyTo('bookings@hopdriver.com', 'HopDriver Bookings');
              $mail->AddBCC('bookings@hopdriver.com');
